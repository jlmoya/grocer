function [grocer_opt,y,namey,x,namexos,z,namexcomp,prests,boundsvarb,nonna,multitest,testfunc,m2prt_test,nz,list_func,optout]=var4auto_explode(grocer_nlag,varargin)
 
// PURPOSE: explode the various arguments entered in function
// automatic when applied to a VAR model
// ------------------------------------------------------------
// INPUT:
...
// * x = (nobs*neqs x k) matrix of exogenous non compulsory
//       variables
// * namex = (k x 1) string collecting their name
// * z = (nobs*neqs x ncomp) matrix of exogenous non compulsory
//       variables
...
grocer_optnum=['descent';'wreliab';'dropna';'nlags';'depth']
grocer_comp=[]
...
grocer_names=tlist(['names';'doornhans';'hetero_sq';...
     'arlm';'arch'],'Jarque & Bera',...
     'Doornik & Hansen','hetero x_squared','AR','ARCH')
 
grocer_opt=tlist(['options';'strategy';'descent';'crit';'groups_pval';...
'eta';'gam';'f0_tdo';'t0_tdo';'f0_bup';'alpha';'test';...
'depth';'wreliab';'dropna';'prt';'saturate';'saturate_post';'block search';'thresh_block';...
'block_nmax'],...
'liberal',5,'bic',[],...
0.01,[],[],[],[],[],emptystr(),...
1,[0.3 ; 0.6 ; 0.4 ; 0.7],%f,'final',%f,%f,%f,0.6,128)
...
for grocer_i=grocer_nargin:-1:1
   if typeof(varargin(grocer_i)) == 'string' then
      ...
      if ~isempty(grocer_indeq) then
         ...
         elseif grocer_nameopt == 'thresh_block' then
            execstr('grocer_opt(grocer_nameopt)='+grocer_valopt)
 
         elseif grocer_nameopt == 'block_nmax' then
            execstr('grocer_opt(grocer_nameopt)='+grocer_valopt)
 
         elseif grocer_nameopt == 'itmax' then
            execstr('grocer_itmax='+grocer_valopt)
 
         elseif grocer_nameopt == 'crit' then
            execstr('grocer_crit='+grocer_valopt)
 
         elseif grocer_nameopt == 'exo' then
            grocer_exo=grocer_valopt
 
         elseif grocer_nameopt == 'endo' then
            grocer_endo=str2vec(varargin(grocer_i))
 
         elseif grocer_nameopt == 'comp' then
         ...
         elseif grocer_var == 'newname' then
      ...
      elseif grocer_var == 'block search' then
         grocer_opt('block search')=%t
         varargin(grocer_i)=null()
 
      elseif or(grocer_var == ['nocte' ; 'noconst']) then
         grocer_prescte=%f
         varargin(grocer_i)=null()
...
[mats,names,prests,boundsvarb,nonna]=explon(list(grocer_endo,grocer_comp,varargin),...
['endogenous';'compulsory';'exogenous'],[],%t,grocer_opt('dropna'),%f,[grocer_nlag;0;0])
yall=mats(1)
z=mats(2)
[nobs,nz]=size(z)
...
if grocer_prescte then
   namex=['const' ; names(3)]
   x=[ones(nobs,1) ; mats(3)]
else
   namex=names(3)
   x=mats(3)
end
xlags=mlagb(yall,grocer_nlag)
[nobs,neqs]=size(yall)
if grocer_prescte then
   x=eye(neqs,neqs) .*. [xlags(grocer_nlag+1:nobs,:) , ones(nobs-grocer_nlag,1) , x]
else
   x=eye(neqs,neqs) .*. [xlags(grocer_nlag+1:nobs,:)  , x]
end
z=eye(neqs,neqs) .*. z
y=yall(grocer_nlag+1:nobs,:)
 
namexcomp=[]
if ~isempty(namecomp) then
   for i=1:neqs
      namexcomp=[namexcomp ; 'eq '+string(i)+'.'+namecomp]
   end
end
 
namey2=namey
for i=1:neqs
   if ~isempty(strindex(namey(i),'(')) then
      namey2(i)='('+namey2(i)+')'
   end
end
namelags=[]
for i=1:grocer_nlag
    namelags=[namelags ; namey2+'(-'+string(i)+')']
end
namexos='eq 1.'+[namelags ;namex]
for i=2:neqs
   namexos=[namexos ; 'eq '+string(i)+'.'+[namelags ;namex]]
end
...
if ~exists('grocer_testfunc','local') then
   [testfunc,m2prt_test]=auto_test(grocer_names,grocer_test,nobs,nvar,test_varspec0)
end
...
multitest=lratio0
list_func=list(var4auto_part,var4auto_upd,var4auto_full)
optout=list(grocer_crit,grocer_itmax,grocer_nlag,yall)
