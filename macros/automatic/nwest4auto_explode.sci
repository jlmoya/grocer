function [grocer_opt,y,namey,x,namexos,z,namecomp,prests,boundsvarb,nonna,multitest,testfunc,m2prt_test,nz,list_func,output_list]=nwest4auto_explode(grocer_namey,varargin)
 
// PURPOSE: explode the various arguments entered in function
// automatic when applied to ols with the Newey-West correction
// for standard errors
// ------------------------------------------------------------
// INPUT:
...
grocer_optnum=['descent';'wreliab';'dropna';'depth']
grocer_comp=[]
   ...
   if typeof(varargin(grocer_i)) == 'string' then
      ...
      if size(grocer_var,'*') == 1 then
         ...
         if ~isempty(grocer_indeq) then
            ...
            if grocer_nameopt == 'win' then
               grocer_win=list(evstr(grocer_valopt))
...
if ~exists('grocer_win','local') then
   grocer_win=floor(5*size(y,1)^0.25)
end
multitest=auto_waldf0
 
list_func=list(nwest4auto_part,nwest4auto_upd,nwest4auto_full)
output_list=list(grocer_win)
