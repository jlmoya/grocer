function res=pfixed4auto_upd(res,y,namexos,indx,val,p,ncomp,list_vararg)
...
if isempty(indx) then
   res('namex')=[ grocer_nameid ; namexos([1:size(z,2)])]
else
   res('namex')=[ grocer_nameid ; namexos([1:size(z,2) size(z,2)+indx])]
end
