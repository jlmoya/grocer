function [grocer_opt,grocer_y,grocer_namey,grocer_x,grocer_namexos,grocer_z,grocer_namecomp,prests,grocer_boundsvarb,nonna,multitest,testfunc,m2prt_test,nz,list_func,output_list]=pfixed4auto_explode(grocer_namey,varargin)
...
grocer_opt_pfixed=tlist(['hac';'alpha';'nboot'])
grocer_subnameid = %f
grocer_hac=0
grocer_nboot=0
grocer_alpha=0
...
if grocer_hac==0 then
  list_func=list(pfixed4auto_part,pfixed4auto_upd,pfixed4auto_full)
else
  ...
  list_func=list(pfixedhac4auto_part,pfixed4auto_upd,pfixedhac4auto_full)
end
...
testfunc=test_empty
m2prt_test=[]
nz=size(grocer_z,2)+size(grocer_nameid,'*')
