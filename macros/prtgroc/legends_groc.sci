...
function legends_groc(leg, style, opt, with_box, font_size )
//
// PURPOSE
...
// http://grocer.toolbox.free.fr/grocer.html
  ...
  if size(opt,'*')>1 then
    ...
    // end bugfix
    opt=0 ;
  elseif opt<1 | opt>7 then
     error('opt can take value in 1 to 7') // if opt is entered by a string by the user it is in the range
  end
  select opt
  case 1 then
  ...
  case 7 then
    // decrease the size of the graph to leave place for the legend
    old_ax.margins=[xmin;arect(2)+width+  xOffset;arect(4);arect(3) ]
    pos=[xmax-width-xOffset, (ymin + ymax)/2+height/2]
  end
