function [rcusum]=cusumb(grocer_namey,varargin)
 
// PURPOSE: compute backward cusum and cusum-squares test and
// plots the tests values along with their 5% confidence bands
// ------------------------------------------------------------
// INPUT:
...
//     size chosen for the test (default =0.05)
...
//   . rcusum('dropna') = boolean indicating if NAs has
//		   been droped
...
vv = recresid(grocer_y(nobs:-1:1),grocer_x(nobs:-1:1,:))
vv=vv(k+1:nobs)
s2 = sum(vv.^2);
...
wu_1 = 1.143*sqrt(df)+2*1.143*[df:-1:1]'/sqrt(df)
wl_1 = -wu_1
wu_5 = .948*sqrt(df)+2*.948*[df:-1:1]'/sqrt(df)
wl_5 = -wu_5
wu_10 = .85*sqrt(df)+2*.85*[df:-1:1]'/sqrt(df)
wl_10 = -wu_10
...
w2l_10=max(-w2u_10+[df:-1:1]'/df,0)
w2u_10=w2u_10+[df:-1:1]'/df
w2l_5=max(-w2u_5+[df:-1:1]'/df,0)
w2u_5=w2u_5+[df:-1:1]'/df
w2l_1=max(-w2u_1+[df:-1:1]'/df,0)
w2u_1=w2u_1+[df:-1:1]'/df
...
,'cusum','backward',nobs,k,grocer_y,grocer_x,vv,...
