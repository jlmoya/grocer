function varargout=robustmod(grocer_model,grocer_tsmat,grocer_indeq,grocer_wfunc,grocer_wparm,varargin)
 
// PURPOSE: compute robust regression using iteratively
// reweighted least-squares for equations of a model
// ------------------------------------------------------------
// INPUT:
...
// * following arguments: a variable number of robust results
...
for grocer_i=grocer_nargin:-1:1
   ...
   if typeof(grocer_argi) == 'string' then
      ...
      elseif grocer_argi == 'dropna' then
         grocer_dropna=%t
         varargin(grocer_i)=null()
      elseif part(varargin(grocer_i),1:4) == 'crit'  then
...
[grocer_model,grocer_listout]=estim_univmod(grocer_model,grocer_indeq,robust1,grocer_save,grocer_prt,grocer_wfunc,grocer_wparm,grocer_crit)
