function [g,dht,gradt,sigt]=garch_grad(parm,nar,nma,y,x)
...
for i = 1:n
   sigt(i+nar) = parm(k+1:k+1+nar+nma)'*[1 ; sigt(i+nar-1:-1:i) ;...
             res(i+nma-1:-1:i).*res(i+nma-1:-1:i)];
   d(i)=1+ar'*d(i+nar-1:-1:i)
