function [simul_result,simul_db]=simul_shock(grocer_model,grocer_db,grocer_startdate,grocer_enddate,varargin)
...
for grocer_i=grocer_nargin:-1:1
   if typeof(varargin(grocer_i)) == 'string' then
      ...
      if part(grocer_argi,1:5) == 'from=' then
         grocer_fromdate=part(grocer_argi,6:length(grocer_argi))
         varargin(grocer_i) = null()
      elseif part(grocer_argi,1:3) == 'to=' then
         grocer_fromdate=part(grocer_argi,4:length(grocer_argi))
         varargin(grocer_i) = null()
      elseif part(grocer_argi,1:4) == 'lag=' then
...
for grocer_i=grocer_nargin:-1:1
   ...
   case 'list' then
      ...
      for grocer_j=1:size(grocer_vari,'*')
         ...
         case 'ts' then
            ...
            error('not an available option for a shock: '+grocer_type_shock)
         end
      end
      varargin(grocer_i)=null()
   ...
   case 'string' then
      if part(grocer_argi,1:5) == 'from=' then
         grocer_fromdate=part(grocer_argi,6:length(grocer_argi))
      elseif part(grocer_argi,1:3) == 'to=' then
         grocer_todate=part(grocer_argi,5:length(grocer_argi))
      end
   end
end
grocer_db('series')=grocer_series
[simul_result,simul_db]=simulate(grocer_model,grocer_db,grocer_startdate,grocer_enddate,grocer_lag,grocer_fromdate,grocer_todate,grocer_ftol,grocer_deltol,varargin(:))
