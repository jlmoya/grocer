function res=ologit(grocer_namey,varargin)
 
// PURPOSE: compute multivariate ordered logit regression
// ------------------------------------------------------------
// INPUT:
...
for grocer_i=grocer_nargin:-1:1
   ...
   if typeof(varargin(grocer_i)) == 'string' then
      ...
      if part(grocer_argi2,1:5) == 'init=' then
          execstr('grocer_init'+grocer_argi)
...
if ~exists('grocer_init','local') then
   ...
   for i=1:nvaly-1
      ...
      grocer_init=[grocer_init;-log(1/p-1)]
   end
end
 
res=ologit1(grocer_y,grocer_x,grocer_init,grocer_optfunc,grocer_opt_optim)
