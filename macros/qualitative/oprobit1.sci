function res=oprobit1(y,x,param,optim_func,opt_optim)
...
if nargin == 2 then
   nvaly=size(grocer_y,2)
   beta=ols0(y,x)
end
 
deff('[f,g,ind]=grocer_oprobit(theta,ind,y,x)',...
['[f0,g0]=oprobit_like(theta,y,x)';'f=-f0';'g=-g0'])
deff('[f]=grocer_oprobit0(theta,y,x)',...
['f=-oprobit_like0(theta,y,x)'])
 
select optim_func
case 'optim' then
   execstr('[f,theta,g] = optim(list(grocer_oprobit,y,x)'+...
   +opt_optim('optim ineq')+',param'+opt_optim('optim')+')')
 
case 'optimg' then
   [f,theta,g] = optimg(list(grocer_oprobit0,y,x),list(grocer_oprobit,y,x),param,...
...
[llike,gra,F]=oprobit_like(theta,y,x)
P=F(:,2:$)-F(:,1:$-1)
 
res=tlist(['results';'meth';'y';'x';'nobs';'nvar';'ncat';'beta';...
'yhat';'r2mf';'rsqr';'llike';'grad';'lratio';'condindex'],...
'ordered probit',y,x,nobs,size(theta,1),nvaly,theta,P,r2mf,rsqr,...
-f,-g,lratio,bkwols(x))
res = hessoprobit(res);
