function ts=lagda(varargin)
 
// PURPOSE: Compute x(t-n) for a timeseries or a tsmat x,
//  while ignoring na values
// ------------------------------------------------------------
// INPUT:
// * first argument = the lag order (optional, set to 1 if not
// provided, can be >0, =0 or <0)
// * last argument (which can be the first !) = a time series
// ------------------------------------------------------------
// OUTPUT:
// * ts = the lagged time series or the lagged tsmat
// ------------------------------------------------------------
...
s=s*%nan
s(findnonna(l1:ln,:))=vec(snonna(l0:ln1,:))
ts('series')=s
...
if (typeof(ts)=='tsmat') then
   ...
   case 'trace' then
      ts('names')='lag('+string(l)+','+ts('names')+')'
