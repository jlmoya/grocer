function tsmat=%tsmat_cumsum(tsmat)
...
// cumsum(tsmat) to take the cumulative product of tsmat
// ------------------------------------------------------------
// INPUT:
...
for i=1:k
   ...
   if indnonna == [] then
      ...
      s(firstnonna:size(s,1),i)=cumsum(s(firstnonna:size(s,1),i))
   end
end
...
case 'trace' then
   tsmat('names') = 'cumsum('+tsmat('names')+')'
