function [llike,sigma_a,bet,w,U,sqrtWi]=chowlin_like(rho,Y,X,LL,C,N,evec)
 
// PURPOSE: Log-likelihood for the Chow-Lin desaggregation
// method
// ------------------------------------------------------------
// INPUT:
...
[u,d,v]=svd(W)
sqrtWi = real(u* diag(evec ./ sqrt(real(diag(d)))) *v')
wi=sqrtWi*sqrtWi
bet = ols0(sqrtWi*Y,sqrtWi*X)
// beta ML estimator
U = Y-X*bet
// Low frequency residuals
sigma_a = real(U'*wi*U/N)
