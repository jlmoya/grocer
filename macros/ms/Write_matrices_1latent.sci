function [PZ,PX_c]=Write_matrices_1latent(vect,nbstates,nojump);
...
if nojump then
   ...
   PZ(1,1)=(PZ(1,1)/(1+PZ(1,1))+eps)/(1+2*eps)
   PZ(2,1)=1-PZ(1,1)
   for i=2:nbstates-1
      PZ(i-1,i)=(vect(nbstates+i-2)/(1+vect(nbstates+i-2)+PZ(i,i))+eps)/(1+3*eps)
      PZ(i,i)=(PZ(i,i)/(1+vect(nbstates+i-2)+PZ(i,i))+eps)/(1+3*eps)
      PZ(i+1,i)=1-PZ(i,i)-PZ(i-1,i)
   end
   indvect=2*nbstates-2
   PZ(nbstates-1,nbstates)=(vect(indvect)/(1+vect(indvect))+eps)/(1+2*eps)
   ...
   PZ(1:nbstates-1,1:nbstates)=PZ(1:nbstates-1,1:nbstates) ./ ...
   (1+sum(PZ(1:nbstates-1,1:nbstates),'r') .*. ones(nbstates-1,1))
...
for i=1:nvar
   ...
   (1+sum(vect(indvect,:),'r') .*. ones(nbqua(1)-1,1))
