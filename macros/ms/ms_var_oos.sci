function grocer_res_inp=ms_var_oos(grocer_res_inp,grocer_endo,varargin)
 
// PURPOSE: run out-of-sample an estaimated Markov-switching
// VAR
//------------------------------------------------------------
// INPUT:
...
// * grocer_res_inp('meth') ='out of sample ms var'
...
// * grocer_res_inp('coeff') = the (np x 1) vector of parameters
...
// * grocer_res_inp('prob_st') = the (M x 1) vector of
//   egodic state probabilities
// * grocer_res_inp('ptrans') = the (M x M) matrix of
//   transition probabilities
// * grocer_res_inp('sigma') = the (M*M_V x M)
//   variance-covariance matrix of the residuals
// * grocer_res_inp('beta_id') = the (1 x sum(n_x)*M) vector
...
//   associated p-values
// * grocer_res_inp('covbeta') = the (np x np)
...
//   t-stats for the variance-covariance matrix of the residuals
// * grocer_res_inp('smoothed resid') = the smoothed residuals of
//   the regression
...
[grocer_yall,grocer_namey,grocer_prests,grocer_b,nonna]=explone(grocer_endo,[],'endogenous',%t,grocer_dropna,%f,grocer_MS_p)
...
if grocer_MS_typmod == 2 then
   ...
   grocer_z = zeros(grocer_MS_K*grocer_MS_T,1)
...
elseif grocer_MS_typmod == 3 then
   ...
   error('not a valid typemod for ms_var_oos input tlist')
end
...
Mu=grocer_x*grocer_res_inp('beta_id')+grocer_z*(ones(1,grocer_MS_M) .*. grocer_res_inp('beta_co'));
...
grocer_res_inp('xmat')=grocer_x
grocer_res_inp('zmat')=grocer_z
