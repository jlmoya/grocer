function grocer_res_inp=ms_reg_oos(grocer_res_inp,grocer_endo,grocer_exo_com,grocer_exo_idio,varargin)
 
// PURPOSE: run out-of-sample an estimated Markov-switching
// regression model
//------------------------------------------------------------
// INPUT:
// * grocer_res_inp = a 'ms reg' results tlist
// * grocer_endo =
...
// * grocer_res_inp('meth') ='out of sample ms reg'
...
[grocer_mats,grocer_names,grocer_prests,grocer_b,grocer_nonna]=...
   ...
   ['endogenous' 'idio exo' 'com exo'],[],%t,grocer_dropna)
 
y=grocer_mats(1)
[grocer_MS_T,grocer_MS_K]=size(y)
y_mat=matrix(y,grocer_MS_T*grocer_MS_K,1)
 
if isempty(grocer_exo_com) then
   grocer_exo_com=zeros()
end
...
if grocer_nb_zlists == 1 then
   ...
   cums_nz=cumsum([0 ; nz])
   if cums_nz($) == 0 then
      z=zeros(grocer_MS_T*grocer_MS_K,1)
   else
...
Mu=x*grocer_res_inp('beta_id')+z*(ones(1,grocer_MS_M) .*. grocer_res_inp('beta_co'));
